...
class athDataVTK:
    ...
    def loadSimDataVTK(self,fileN):
        ...
        print("dimensions (nx, nt, nz) :", self.vec)

        self.nx = dim[0]
        self.nt = dim[1]
        self.nz = dim[2]

        self.x = nm.zeros(self.data.GetNumberOfPoints())
        ...
        for i in range(0,self.data.GetNumberOfPoints()):
            self.x[i],self.t[i],self.z[i]= self.data.GetPoint(i)
        
        ord = 'F'
        # ord = 'C'

        if ord == 'F':
            self.x = self.x.reshape(dim,order='F')
            self.t = self.t.reshape(dim,order='F')
            self.z = self.z.reshape(dim,order='F')

        else:
            self.x = self.x.reshape(dim,order='C')
